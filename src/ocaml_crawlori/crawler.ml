...
let dummy_config =
  {
    nodes = ["https://tz.functori.com"];
    sleep = 6.;
    start = Some 13650000l;
    forward = None;
    verbose = 0;
    db_kind = `pg;
    step_forward = 20;
    confirmations_needed = 10l;
    plugins = [];
    retry = Some 5;
    retry_timeout = 10.;
    no_forward = false;
    extra = dummy_extra_config;
  }
...
let usage =
  ...
  @@ EzEncoding.construct
       ~compact:false
       (config_enc extra_config_enc)
       dummy_config

let crawler config = Register.register_all_plugins config
          ...
          let config =
            {
              ...
              extra =
                {
                  E.contracts = CSet.of_list config.extra;
                  originator_address = Option.get ext#originator_address;
                };
            }
          in
