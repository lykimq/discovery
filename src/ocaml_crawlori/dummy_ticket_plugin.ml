...
let insert_dummy_ticket_storage ?(forward = false) ~dbh ~contract ~op parameter
  ...
  | Some storage ->
      ...
      [%pgsql
        dbh
          "insert into \
           dummy_ticket_storage(contract,transaction,block,index,level,tsp,main,storage_content) \
           values \
           ($contract,${op.bo_hash},${op.bo_block},${op.bo_index},${op.bo_level},${op.bo_tsp},$forward,$p)"]

let insert_dummy_ticket_burn_callback ?(forward = false) ~dbh ~contract ~op
    parameter =
  let p = EzEncoding.construct micheline_enc.json parameter in
  [%pgsql
    dbh
      "insert into \
       dummy_ticket_burn_callback(contract,transaction,block,index,level,tsp,main,burn_callback_parameter) \
       values \
       ($contract,${op.bo_hash},${op.bo_block},${op.bo_index},${op.bo_level},${op.bo_tsp},$forward,$p)"]

let insert_dummy_ticket_mint_to_deku ?(forward = false) ~dbh ~contract ~op
    parameter =
  let p = EzEncoding.construct micheline_enc.json parameter in
  [%pgsql
    dbh
      "insert into \
       dummy_ticket_mint_to_deku(contract,transaction,block,index,level,tsp,main,mint_to_deku_parameter) \
       values \
       ($contract,${op.bo_hash},${op.bo_block},${op.bo_index},${op.bo_level},${op.bo_tsp},$forward,$p)"]

let insert_dummy_ticket_withdraw_from_deku ?(forward = false) ~dbh ~contract ~op
    parameter =
  let p = EzEncoding.construct micheline_enc.json parameter in
  [%pgsql
    dbh
      "insert into \
       dummy_ticket_withdraw_from_deku(contract,transaction,block,index,level,tsp,main,withdraw_from_deku_parameter) \
       values \
       ($contract,${op.bo_hash},${op.bo_block},${op.bo_index},${op.bo_level},${op.bo_tsp},$forward,$p)"]

let get_storage_bms_id ?(allocs = []) ~fields ~storage_type storage_value =
  ignore (allocs, fields, storage_type, storage_value) ;
  Ok ()
...
let get_storage_ori ~allocs script =
  try
    ...
    in
    let$ () = get_storage_bms_id ~allocs ~fields ~storage_type storage_value in
    Ok storage
  with exn ->
  ...
  if op.bo_op.source = config.originator_address then
    ...
    | Some meta -> (
        ...
        | Ok s ->
            ...
            Contracts.dummy_ticket_register
              ~forward
              ~dbh
              config
              address
      ...
      | EPnamed "withdraw_from_deku" ->
          let>? () =
            Contracts.insert_all_operations
              ...
              ~op
              ~contract
              "dummy_ticket_withdraw_from_deku"
          in
          insert_dummy_ticket_withdraw_from_deku
            ?forward
            ~dbh
            ~contract
            ~op
...
let register_operation ?forward info dbh op =
  ...
  | Transaction {destination = contract; parameters; _}, Some meta -> (
      match
        List.find_opt (fun c ->
            c.ci_address = contract && c.ci_kind = "dummy_ticket")
