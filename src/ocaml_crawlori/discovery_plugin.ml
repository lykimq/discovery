...
match parameter with
  ...
  | Some storage ->
    ...
    [%pgsql dbh "insert into discovery_storage(contract,transaction,block,index,level,tsp,main,storage_content) values ($contract,${op.bo_hash},${op.bo_block},${op.bo_index},${op.bo_level},${op.bo_tsp},$forward,$p)"]
...
[%pgsql dbh "insert into discovery__default(contract,transaction,block,index,level,tsp,main,_default_parameter) values ($contract,${op.bo_hash},${op.bo_block},${op.bo_index},${op.bo_level},${op.bo_tsp},$forward,$p)"]
...
iter (fun (k, v) ->
       ...
       [%pgsql dbh "insert into bm_discovery_(contract,transaction,block,index,level,tsp,main,key__parameter,value__parameter) values ($contract,${op.bo_hash},${op.bo_block},${op.bo_index},${op.bo_level},${op.bo_tsp},$forward,$k,$v)"]) parameter

let get_storage_bms_id ?(allocs=[]) ~fields ~storage_type storage_value =
    let$ bmid_discovery_ =
  match Bm_utils.find_opt_storage_field
    ~allocs ~fields ~storage_type ~storage_value "" with
| _, Some { bm_id ; _} -> Ok bm_id
| _ -> Error (`generic ("parse_error", "storage.")) in
Ok(bmid_discovery_)
...
let get_bmid_discovery__updates bm_id l =
    ...
    match op.bo_meta with
      ...
      | Some meta ->
        ...
        | Ok (s,bmid_discovery_) ->
          ...
          let>? () = insert_discovery_storage ~dbh ~forward ~contract:address ~op storage in
          let>? () =
            Contracts.insert_all_operations
              ~forward ~dbh ~op ~contract:address "origination" in
          let bmid_discovery__updates = get_bmid_discovery__updates bmid_discovery_ meta.op_lazy_storage_diff in
let>? () = insert_bm_discovery_ ~dbh ~forward ~op ~contract:address bmid_discovery__updates in
          Contracts.discovery_register ~forward ~dbh config address "discovery" op bmid_discovery_
    end
  else rok ()
...
let register_block ?forward config dbh b =
  ...
  | Some p ->
    ...
    | EPnamed "_default" ->
      let>? () = Contracts.insert_all_operations ?forward ~dbh ~op ~contract "discovery__default" in
  ...
  | Transaction {destination=contract; parameters ; _}, Some meta ->
    match List.find_opt (fun c -> c.ci_address = contract && c.ci_kind = "discovery") @@ CSet.elements info.contracts with
    | None -> rok ()
    | Some info ->
      ...
      let bmid_discovery__updates = get_bmid_discovery__updates (Option.get info.ci_bmid_discovery_) meta.op_lazy_storage_diff in
let>? () = insert_bm_discovery_ ~dbh ?forward ~op ~contract bmid_discovery__updates in
