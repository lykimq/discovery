...
type bigmap_info = {bm_id : z; bm_types : bigmap_types} [@@deriving encoding]
...
type extra = {mutable contracts : CSet.t [@set contract_info_enc]}
[@@deriving encoding]
...
let short ?(len = 8) h = if String.length h > len then String.sub h 0 len else h

let use dbh f = match dbh with None -> PG.Pool.use f | Some dbh -> f dbh
