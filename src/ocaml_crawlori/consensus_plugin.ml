...
let insert_consensus_storage ?(forward = false) ~dbh ~contract ~op parameter =
  ...
  | Some storage ->
      ...
      [%pgsql
        dbh
          "insert into \
           consensus_storage(contract,transaction,block,index,level,tsp,main,storage_content) \
           values \
...
let insert_consensus_deposit ?(forward = false) ~dbh ~contract ~op parameter =
  ...
  [%pgsql
    dbh
      "insert into \
       consensus_deposit(contract,transaction,block,index,level,tsp,main,deposit_parameter) \
       values \
       ($contract,${op.bo_hash},${op.bo_block},${op.bo_index},${op.bo_level},${op.bo_tsp},$forward,$p)"]

let insert_consensus_update_root_hash ?(forward = false) ~dbh ~contract ~op
    parameter =
  let p = EzEncoding.construct micheline_enc.json parameter in
  [%pgsql
    dbh
      "insert into \
       consensus_update_root_hash(contract,transaction,block,index,level,tsp,main,update_root_hash_parameter) \
       values \
       ($contract,${op.bo_hash},${op.bo_block},${op.bo_index},${op.bo_level},${op.bo_tsp},$forward,$p)"]
...
let insert_consensus_withdraw ?(forward = false) ~dbh ~contract ~op parameter =
  ...
  [%pgsql
    dbh
      "insert into \
       consensus_withdraw(contract,transaction,block,index,level,tsp,main,withdraw_parameter) \
       values \
       ($contract,${op.bo_hash},${op.bo_block},${op.bo_index},${op.bo_level},${op.bo_tsp},$forward,$p)"]
  ...
  iter
    (fun (k, v) ->
      ...
      [%pgsql
        dbh
          "insert into \
           bm_consensus_vault(contract,transaction,block,index,level,tsp,main,key_vault_parameter,value_vault_parameter) \
           values \
           ($contract,${op.bo_hash},${op.bo_block},${op.bo_index},${op.bo_level},${op.bo_tsp},$forward,$k,$v)"])
    parameter

let insert_bm_consensus_used_handles ?(forward = false) ~dbh ~contract ~op
    parameter =
  iter
    (fun (k, v) ->
      let k = EzEncoding.construct micheline_enc.json k in
      ...
      [%pgsql
        dbh
          "insert into \
           bm_consensus_used_handles(contract,transaction,block,index,level,tsp,main,key_used_handles_parameter,value_used_handles_parameter) \
           values \
           ($contract,${op.bo_hash},${op.bo_block},${op.bo_index},${op.bo_level},${op.bo_tsp},$forward,$k,$v)"])
    parameter

let insert_bm_consensus_known_handles_hash ?(forward = false) ~dbh ~contract ~op
    parameter =
  iter
    (fun (k, v) ->
      let k = EzEncoding.construct micheline_enc.json k in
      ...
      [%pgsql
        dbh
          "insert into \
           bm_consensus_known_handles_hash(contract,transaction,block,index,level,tsp,main,key_known_handles_hash_parameter,value_known_handles_hash_parameter) \
           values \
           ($contract,${op.bo_hash},${op.bo_block},${op.bo_index},${op.bo_level},${op.bo_tsp},$forward,$k,$v)"])
    parameter
...
let get_storage_bms_id ?(allocs = []) ~fields ~storage_type storage_value =
  let$ bmid_consensus_vault_vault =
    ...
    | _ -> Error (`generic ("parse_error", "storage.vault"))
  in
  let$ bmid_consensus_vault_used_handles =
    ...
    with
    | _, Some {bm_id; _} -> Ok bm_id
    | _ -> Error (`generic ("parse_error", "storage.used_handles"))
  in
  let$ bmid_consensus_vault_known_handles_hash =
    ...
    with
    | _, Some {bm_id; _} -> Ok bm_id
    | _ -> Error (`generic ("parse_error", "storage.known_handles_hash"))
  in
  Ok
...
let get_bmid_consensus_vault_vault_updates bm_id l =
  Bm_utils.get_bm_updates ~bm_id l
        ...
        | Ok
            ...
            let>? () =
              Contracts.insert_all_operations
                ...
                "origination"
            in
            let bmid_consensus_vault_vault_updates =
              get_bmid_consensus_vault_vault_updates
                bmid_consensus_vault_vault
                meta.op_lazy_storage_diff
            in
            let>? () =
              insert_bm_consensus_vault
                ~dbh
                ~forward
                ~op
                ~contract:address
                bmid_consensus_vault_vault_updates
            in
            let bmid_consensus_vault_used_handles_updates =
              get_bmid_consensus_vault_used_handles_updates
                bmid_consensus_vault_used_handles
                meta.op_lazy_storage_diff
            in
            let>? () =
              insert_bm_consensus_used_handles
                ~dbh
                ~forward
                ~op
                ~contract:address
                bmid_consensus_vault_used_handles_updates
            in
            let bmid_consensus_vault_known_handles_hash_updates =
              get_bmid_consensus_vault_known_handles_hash_updates
                bmid_consensus_vault_known_handles_hash
                meta.op_lazy_storage_diff
            in
            let>? () =
              insert_bm_consensus_known_handles_hash
                ~dbh
                ~forward
                ~op
                ~contract:address
                bmid_consensus_vault_known_handles_hash_updates
            in
            Contracts.consensus_register
              ~forward
              ~dbh
              config
              address
...
let register_operation ?forward info dbh op =
  ...
  | Transaction {destination = contract; parameters; _}, Some meta -> (
      match
        List.find_opt (fun c ->
            c.ci_address = contract && c.ci_kind = "consensus")
      ...
      | Some info ->
          ...
          let bmid_consensus_vault_vault_updates =
            get_bmid_consensus_vault_vault_updates
              (Option.get info.ci_bmid_consensus_vault_vault)
              meta.op_lazy_storage_diff
          in
          let>? () =
            insert_bm_consensus_vault
          ...
          let bmid_consensus_vault_used_handles_updates =
            get_bmid_consensus_vault_used_handles_updates
              (Option.get info.ci_bmid_consensus_vault_used_handles)
              meta.op_lazy_storage_diff
          in
          let>? () =
            insert_bm_consensus_used_handles
          ...
          let bmid_consensus_vault_known_handles_hash_updates =
            get_bmid_consensus_vault_known_handles_hash_updates
              (Option.get info.ci_bmid_consensus_vault_known_handles_hash)
              meta.op_lazy_storage_diff
          in
          let>? () =
            insert_bm_consensus_known_handles_hash
