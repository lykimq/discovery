...
let withdraw_from_deku_encode arg =
  Mprim
    {
      prim = `Pair;
      args =
          ...
          Mprim
            {
              prim = `Pair;
              args =
                [address_encode arg.deku_consensus; handle_encode arg.handle];
              annots = [];
            };
...
let withdraw_from_deku_decode (m : micheline) : withdraw_from_deku =
  let (deku_consensus, handle), handles_hash, proof =
    (tuple3_decode
       (tuple2_decode address_decode handle_decode)
       bytes_decode
...
let withdraw_from_deku_micheline =
  Mprim
    {
      prim = `pair;
      args =
...
let withdraw_from_deku_generator () =
  {
    proof =
      (list_generator (tuple2_generator bytes_generator bytes_generator)) ();
    handles_hash = bytes_generator ();
    handle = handle_generator ();
    deku_consensus = address_generator ();
  }

let call_withdraw_from_deku ?(node = Blockchain.default_node) ?(debug = false)
    ?(amount = 0L) ~from ~kt1 (param : withdraw_from_deku) =
  let param =
    {
          ...
          Mprim
            {
              ...
              args =
                [
                  address_encode arg.deku_consensus;
                  address_encode arg.deku_recipient;
                ];
              annots = [];
            };
          nat_encode arg.ticket_amount;
          bytes_encode arg.ticket_data;
        ];
      annots = [];
    }

let mint_to_deku_decode (m : micheline) : mint_to_deku =
  let (deku_consensus, deku_recipient), ticket_amount, ticket_data =
    (tuple3_decode
       (tuple2_decode address_decode address_decode)
       nat_decode
       bytes_decode)
      m
  in
...
let mint_to_deku_generator () =
  {
    ticket_data = bytes_generator ();
    ticket_amount = nat_generator ();
    deku_recipient = address_generator ();
    deku_consensus = address_generator ();
  }

let call_mint_to_deku ?(node = Blockchain.default_node) ?(debug = false)
    ?(amount = 0L) ~from ~kt1 (param : mint_to_deku) =
  let param =
    {
...
let burn_callback_encode (arg : burn_callback) : micheline =
  (ticket_encode bytes_encode) arg
...
let call_burn_callback ?(node = Blockchain.default_node) ?(debug = false)
    ?(amount = 0L) ~from ~kt1 (param : burn_callback) =
  let param =
    {
