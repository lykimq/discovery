...
let _default_encode arg = Mprim {prim = `Pair;
args = [Mprim {prim = `Pair;
args = [(key_encode arg.key);(int_michelson_encode arg.nonce)];
annots=[]}
;(signature_encode arg.signature);(string_encode arg.uri)];
annots=[]}

let _default_decode (m : micheline) : _default =
let ((key,nonce),signature,uri) = (tuple3_decode (tuple2_decode key_decode int_michelson_decode) signature_decode string_decode) m in
{uri : string;signature : signature;nonce : int_michelson;key : key}

let _default_micheline = Mprim {prim = `pair;
args = [Mprim {prim = `pair;
args = [(key_micheline);(int_michelson_micheline)];
annots=[]}
;(signature_micheline);(string_micheline)];
annots=[]}

let _default_generator () = {uri = string_generator ();signature = signature_generator ();nonce = int_michelson_generator ();key = key_generator ()}
...
type storage = (key_hash,(int_michelson *
 string)) big_map
let storage_encode : storage -> micheline = (big_map_encode (key_hash_encode) ((tuple2_encode int_michelson_encode
string_encode)))
let storage_decode = (big_map_decode (key_hash_decode) ((tuple2_decode int_michelson_decode
string_decode)))
let storage_micheline = (big_map_micheline (key_hash_micheline) ((tuple2_micheline int_michelson_micheline
string_micheline)))
let storage_generator () = (big_map_generator (key_hash_generator) ((tuple2_generator int_michelson_generator
string_generator))) ()
